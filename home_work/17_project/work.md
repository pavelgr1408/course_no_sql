...
Поресёрчил тему, посоветовался с ИИ и понял, что прямое сравнение здесь
...
брокерами было нечестно. Поресёрчил подходы, посоветовался с ИИ, и пришёл к
...
каждого мастера по одной реплике, и `WAIT 2` будет ждать вторую вечность. Поресёрчил вопрос, и оказалось, что для durability-эксперимента с WAIT корректная схема
...
Поковырялся в логах, посоветовался с ИИ, тот помог разобрать вывод
`docker logs nats1`:
